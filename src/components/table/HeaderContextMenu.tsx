      ...
      {items.map((item, idx) => {
        ...
        if (item.children && item.children.length > 0) {
          ...
          return (
            ...
            >
              <DisclosureButton
                className="rounded-none px-3 py-1.5 text-foreground hover:bg-muted"
                role="menuitem"
                aria-haspopup="menu"
              ...
              >
                ...
                <ChevronGlyphIcon className="h-3.5 w-3.5 text-muted-foreground" direction="right" />
              </DisclosureButton>
              {subOpen && (
                <div
                  className="absolute left-full top-0 z-50 min-w-[180px] rounded-md border border-border bg-card py-1 shadow-lg"
                  role="menu"
                  aria-label={item.label}
                >
                  {item.children.map((child) => {
                    if (child.separator) {
                      return <div key={`sub-sep-${child.label}`} className="my-1 h-px bg-muted" role="separator" />;
                    }
                    return (
                      <MenuAction
                        key={child.label}
                        className="rounded-none px-3 py-1.5 text-foreground hover:bg-muted disabled:cursor-not-allowed disabled:text-muted-foreground"
                        role="menuitemradio"
