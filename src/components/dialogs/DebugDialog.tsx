...
function CollapsibleSection({ title, children, defaultOpen = false }: CollapsibleSectionProps) {
  ...
  return (
    <div className="border border-border rounded mb-2">
      <DisclosureButton
        className="rounded-none px-3 py-2 text-foreground hover:bg-muted"
...
function JsonView({ data }: { data: unknown }) {
  ...
  return (
    <pre className="text-xs font-mono bg-muted p-2 rounded overflow-auto max-h-60 whitespace-pre-wrap break-words">
      {text}
    </pre>
  );
}
          ...
          <TabsContent value="view" className="pt-4 space-y-2">
            <CollapsibleSection title={t('DEBUG.VIEW_CONFIG') || 'Current Config'} defaultOpen>
              ...
              <div className="mt-2 space-y-2">
                <div>
                  <span className="text-xs font-medium text-muted-foreground">{t('DEBUG.VIEW_FILTER') || 'Filter Config'}</span>
                  <JsonView data={view.filter ?? null} />
                </div>
                <div>
                  <span className="text-xs font-medium text-muted-foreground">{t('DEBUG.VIEW_GROUP') || 'Group Config'}</span>
                  <JsonView data={view.group ?? null} />
                </div>
                <div>
                  <span className="text-xs font-medium text-muted-foreground">{t('DEBUG.VIEW_PIVOT') || 'Pivot Config'}</span>
                  <JsonView data={view.pivot ?? null} />
                </div>
                <div>
                  <span className="text-xs font-medium text-muted-foreground">{t('DEBUG.VIEW_AGGREGATE') || 'Aggregate Config'}</span>
            ...
            {prefs ? (
              <>
                <CollapsibleSection title={t('DEBUG.PREFS_CONFIG') || 'Configuration'} defaultOpen>
                  ...
                  {prefs.bardo != null && (
                    <div className="mt-2">
                      <span className="text-xs font-medium text-muted-foreground">{t('DEBUG.PREFS_BARDO') || 'Bardo'}</span>
                  ...
                  {prefs.perspectives ? (
                    <div className="space-y-2">
                      {Object.entries(prefs.perspectives).map(([id, p]) => (
                        <div key={id} className="border border-border rounded p-2">
                    ...
                    <p className="text-sm text-muted-foreground italic">{t('DEBUG.NO_PERSPECTIVES') || 'No perspectives'}</p>
                  )}
                </CollapsibleSection>
              </>
            ) : (
