...
export function FieldPill({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`wcdv-field-pill inline-flex items-center gap-1 bg-card border border-border rounded px-2 py-0.5 text-xs shadow-sm cursor-grab active:cursor-grabbing ${
        isDragging ? 'ring-2 ring-primary-300 dark:ring-primary-600' : ''
      }`}
      aria-label={`${label}${subtitle ? ` (${subtitle})` : ''}`}
      {...attributes}
      {...listeners}
    >
      <span className="text-muted-foreground select-none" aria-hidden="true"><DragHandleIcon className="h-3.5 w-3.5" /></span>
      <span className="truncate max-w-[120px]">{label}</span>
      {subtitle && (
        <span className="text-primary-500 dark:text-primary-400 text-[10px] font-medium">({subtitle})</span>
      )}
      {showFunctionButton && (
        <Tooltip content={t('CONTROL.GROUP_FUNCTION') || 'Group function'}>
          <Button
            size="sm"
            variant="ghost"
            className={`!p-0 !min-w-0 !h-4 !w-4 ${subtitle ? 'text-primary-500 dark:text-primary-400' : 'text-muted-foreground'} hover:text-primary-600 dark:hover:text-primary-400`}
            onClick={(e) => {
              e.stopPropagation();
              onFunctionClick?.(id);
            }}
